...
export default function DashboardScreen() {
  ...
  async function fetchData() {
    ...
    const [{ data: txData }, { data: budgetData }, { data: lastTxData }] =
      await Promise.all([
        supabase
          .from('transactions')
          .select('*')
        ...
        {budget ? (
          <>
            <Text style={styles.trendText}>
              예산 대비{' '}
              <Text style={{ color: usedRatio > 80 ? '#EF4444' : '#2563EB' }}>
                {usedRatio}% 사용
              </Text>
          ...
          <View style={styles.pillRow}>
            {topCategories.map(({ category, ratio }) => (
              <View
                ...
                style={[
                  styles.pill,
                  { backgroundColor: (CATEGORY_COLORS[category] ?? '#2563EB') + '22' },
                ]}
              >
...
const styles = StyleSheet.create({
  ...
  goalSub: { fontSize: 13, color: '#9CA3AF', marginTop: 4 },
